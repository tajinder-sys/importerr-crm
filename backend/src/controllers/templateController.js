...
const aiGenerateTemplate = async (req, res) => {
  try {
    ...
    // Check DB cache by description (case-insensitive)
    const existing = await Template.findOne({
      type: 'email',
      aiDescription: { $regex: new RegExp(`^${description}$`, 'i') },
    }).lean();
    if (existing) return sendSuccess(res, 'Template fetched from cache', { ...existing, fromCache: true });
...
Keep it professional and concise. 4-8 blocks max.`;
    ...
    const slug = normalizeSlug(parsed.name + '-' + Date.now());
    ...
    return sendSuccess(res, 'Template generated by AI', { ...template.toObject(), fromCache: false });
  } catch (error) {
    if (error?.code === 11000) return sendBadRequest(res, 'Template name or slug already exists');
